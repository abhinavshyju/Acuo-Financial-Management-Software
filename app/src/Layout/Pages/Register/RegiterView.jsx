...
const RegiterView = () => {
        ...
        const CheckDate = sortDate.slice(5,7) +'/'+ sortDate.slice(8,10)+ '/'+ sortDate.slice(0,4)
                ...
                <tbody className="text-gray-600 divide-y">
                    {
                         nameSearch.map((user, index) => (
                            <tr key={user.id} className=''>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap font-bold">{user.username}</td>
                                ...
                                {AddFunc(user.savings, user.loan, user.magalam, user.fine, user.interest)|| "0"}
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                {(user.attendence === 0) ?
                                ...
                                <button className='"py-2 px-3 font-medium text-indigo-600 hover:text-indigo-500 duration-150 hover:bg-gray-50 rounded-sm'>
                                    <Link to={`/log/edit/${user.id}`}>Edit</Link></button>
                                </td>
                            </tr>
                        ...
                        <tr className='bg-gray-200'>
                            ...
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>{nameSearch.reduce((total, e) => total + Number(e.savings), 0)}</td>
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>{nameSearch.reduce((total, e) => total + Number(e.loan), 0)}</td>
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>{nameSearch.reduce((total, e) => total + Number(e.magalam), 0)}</td>
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>{nameSearch.reduce((total, e) => total + Number(e.fine), 0)}</td>
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>{nameSearch.reduce((total, e) => total + Number(e.interest), 0)}</td>
                            <td className='px-4 py-3 whitespace-nowrap font-bold'>
                                {nameSearch.length > 0
