...
const AddAddtionalLoan = () => {
  ...
  const findInterestTotal = (LoanAmount, interestRate, Months) => {
    const total =
      Number(LoanAmount) +
        Number(((LoanAmount * (interestRate / 100)) / 12) * Months) || 0;
    return total;
  };
  const submit = (event) => {
    ...
    if (id) {
      const loan_total =
        Number(LoanAmount) +
        Number(((LoanAmount * (interestRate / 100)) / 12) * Months);
      const data = {
        loan: loan_total,
        id: id,
      ...
      window.BackendAPI.send("add_additional_loan", data);
  ...
  return (
    <div className="mx-auto px-4 md:px-6 mt-10">
      ...
      <form
        onSubmit={submit}
        className="container flex flex-col mx-auto space-y-12 mt-10 border shadow-md rounded-md pb-5"
      >
        <fieldset className="grid grid-cols-4 gap-6 p-6 rounded-md shadow-sm bg-white">
          <div className="col-span-full">
            <div className="text-2xl font-bold">Add New Additional Loan</div>
          </div>
          <div className="grid grid-cols-6 gap-4 col-span-full">
            ...
            <div className="col-span-full md:col-span-3">
              ...
              <input
                ...
                value={LoanAmount}
                onChange={(e) =>
                  SetLoanAmount(e.target.value || false || false)
                }
                required
              />
            </div>
...
export default AddAddtionalLoan;
