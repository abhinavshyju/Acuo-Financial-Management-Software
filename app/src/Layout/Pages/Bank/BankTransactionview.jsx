...
import BankSubNAv from '../../Components/NanoComponents/BankSubNAv';
...
const BankTransactionview = () => {
        ...
        const toDay = Date.slice(6,10) +'-'+ Date.slice(0,2)+ '-'+ Date.slice(3,5)
        const [sortDate, SetsortDate] = useState(toDay);



        const SortDate = (sortDate.slice(5,7)+"/"+sortDate.slice(8,10)+"/"+sortDate.slice(0,4))
        ...
        useEffect(() => {
            async function FetchData() {
                const data = await frontendapi.bankDetails();
                Setexpense(data);
            }
            FetchData();    
        }, []);
    ...
    return (
     ...
     <>
       ...
       <div className="flex mb-8">
           <h1 className='font-bold text-4xl'>Bank Details</h1>
       </div>
        <BankSubNAv id="0"/>
        <div className="items-start justify-between mt-5 md:flex ">
        
        <div className="grid grid-cols-4 gap-5 col-span-full lg:col-span-5 w-full">
               {BankName.map((e)=>(
                 <div className="col-span-1 w-full">
                    <MinCard title={e.bankname} number={e.balance} colour= "bg-[#F5F2FDFF] "/>
                </div>
               ))}
        </div>
    </div>
    ...
    {/* <div className="col-span-full md:col-span-2">
                            <label for="firstname" className="text-sm">Expense Details</label>
                        ...
                        <div className="col-span-full md:col-span-1">
                                  <label for="" className="text-sm">Bank Name</label>
                          ...
                          <div className="col-span-full lg:col-span-1 ml-4">
                              <label for="firstname" className="text-sm">Set Date</label>
                                    <input id="firstname" type="date" className="w-full rounded-md  border-gray-300 text-gray-900" 
                                 placeholder={toDay}
                                 value={sortDate}
                                 onChange={(e)=> SetsortDate(e.target.value)}
                                  />
                      </div>
                        <div className="col-span-full md:col-span-2 flex items-end justify-end">
                        <button
                        onClick={submit}
   ...
   <div className="max-w-lg mt-8 col-span-3">
            ...
            <p className="text-gray-600 mt-2">
            Efficiently log and track all your bank transaction
            </p>
        </div>
   </div>
    <div className="mt-5 shadow-sm border rounded-sm overflow-x-auto ">
        ...
        {[...expense].reverse().slice(limitStart, limitEnd+1 ).map((e, index) => (
            <tr key={e.id }>
            <td className="px-6 py-4 whitespace-nowrap">{index + 1 + limitStart}</td>
            <td className="px-6 py-4 whitespace-nowrap font-bold uppercase">{e.date}</td>
            <td className="px-6 py-4  ">{e.name}</td>
            <td className="px-6 py-4 whitespace-nowrap">{e.bank}</td>
